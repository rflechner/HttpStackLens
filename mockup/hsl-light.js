...
[… body not captured · text/event-stream · streaming …]`,
  ...
  const state = {
    ...
    store: (function () {
      const def = { folder: '~/HttpStackLens/requests', autosave: true, ext: '.http' };
      try { return Object.assign(def, JSON.parse(localStorage.getItem('hsl-store') || '{}')); } catch (e) { return def; }
    })(),
  ...
  function saveStore() {
    try { localStorage.setItem('hsl-store', JSON.stringify(state.store)); } catch (e) {}
    document.dispatchEvent(new CustomEvent('hsl-store-change', { detail: state.store }));
  }
  ...
  function renderSettings() {
    const tabs = [['cert', 'TLS / Certificate'], ['files', 'Request files'], ['body', 'Body capture'], ['upstream', 'Upstream proxy'], ['access', 'Access control'], ['hotkeys', 'Shortcuts']];
  ...
  function settingsBody() {
    ...
    if (settingsTab === 'files') return filesPanel();
  ...
  function filesPanel() {
    const s = state.store;
    const count = (function () { try { return (JSON.parse(localStorage.getItem('hsl-http-files') || '[]')).length; } catch (e) { return 0; } })();
    return `<div class="grid gap-[14px]">
      <div style="font-size:12px;color:${C.dim};line-height:1.6">Requests you edit and replay live in plain <span style="font-family:'JetBrains Mono';color:${C.ink}">.http</span> files — the same format VS Code REST Client and JetBrains HTTP Client use. Pick the folder they are read from and saved to.</div>
      ${field('Default folder', 'New and imported files land here', `<div class="flex gap-[6px]"><input data-storefield="folder" value="${esc(s.folder)}" placeholder="~/HttpStackLens/requests" style="background:${C.bg2};color:${C.ink};border:1px solid ${C.line};border-radius:3px;padding:6px 8px;font-family:'JetBrains Mono';font-size:11.5px;flex:1;outline:none">${btn('Browse…', 'files-browse')}</div>`)}
      <div style="padding:14px;background:${C.bg2};border:1px solid ${C.line};border-radius:4px">
        <div class="flex items-center gap-[10px]"><div class="flex-1"><div style="font-size:12.5px;color:${C.ink};font-weight:600">Save on every edit</div><div style="font-size:11px;color:${C.dim};margin-top:2px">Write changes straight back to the file instead of waiting for ⌘S</div></div>${toggle(s.autosave, 'files-autosave')}</div>
      </div>
      <div style="font-size:11px;color:${C.dim};font-family:'JetBrains Mono';padding:8px 10px;background:${C.bg2};border-radius:3px;border:1px solid ${C.line}">${count} file${count === 1 ? '' : 's'} in workspace · <span style="color:${C.mint}">${esc(s.folder)}</span></div>
      <div style="font-size:11.5px;color:${C.faint};line-height:1.6">Files stay readable and diff-friendly: <span style="font-family:'JetBrains Mono';color:${C.dim}">@variables</span> at the top, one request per <span style="font-family:'JetBrains Mono';color:${C.dim}">###</span> block. Commit them next to the code they exercise.</div></div>`;
  }
  ...
  function wire() {
    ...
    document.addEventListener('input', (e) => {
      ...
      if (f) { state.store[f.dataset.storefield] = f.value; saveStore(); }
    });
  ...
  function handleAction(a) {
    switch (a) {
      ...
      case 'cert-generate':
        ...
        break;
      case 'edit': case 'replay': {
        const r = state.rows.find((x) => x.id === state.selId);
        if (r && window.HSLCompose) window.HSLCompose.fromCaptured({
          method: r.method, scheme: r.scheme, host: r.host, path: r.path,
          headers: reqHeaders(r).filter(([k]) => !k.startsWith(':')),
          body: ['POST', 'PUT', 'PATCH'].includes(r.method) ? bodyFor(r) : '',
        });
        break;
      }
      case 'files-autosave': state.store.autosave = !state.store.autosave; saveStore(); renderSettings(); break;
      case 'files-browse':
        (async () => {
          try {
            if (window.showDirectoryPicker) {
              const h = await window.showDirectoryPicker();
              state.store.folder = h.name; saveStore(); renderSettings();
            }
          } catch (err) {}
        })();
        break;
  ...
  // consumed by the composer: log a request it just sent
  window.HSL = {
    store: () => state.store,
    openFileSettings() { openSettings('files'); },
    pushRequest(o) {
      const id = (state.rows[state.rows.length - 1]?.id || 0) + 1;
      const mime = o.mime || 'application/json';
      const kind = /json/.test(mime) ? 'json' : /html/.test(mime) ? 'html' : /javascript/.test(mime) ? 'js' : /css/.test(mime) ? 'css' : /image/.test(mime) ? 'img' : 'bin';
      state.rows.push({
        id, ts: Date.now(), method: o.method, scheme: o.scheme || 'https', host: o.host, path: o.path,
        status: o.status, mime, mimeColor: kind, size: o.size || 0, ms: o.ms || 0,
        tls: (o.scheme || 'https') === 'https', decrypted: true,
        clientIp: '127.0.0.1', process: 'composer', origin: o.origin,
      });
      if (state.rows.length > 220) state.rows.shift();
      renderList();
    },
